...
#undef  Wimp_PlotIconWithWindowSpriteArea
#define Wimp_PlotIconWithWindowSpriteArea       0x400E2
#undef  XWimp_PlotIconWithWindowSpriteArea
#define XWimp_PlotIconWithWindowSpriteArea      0x600E2
#undef  Wimp_PlotIconWithBackgroundSpriteArea
#define Wimp_PlotIconWithBackgroundSpriteArea   0x400E2
#undef  XWimp_PlotIconWithBackgroundSpriteArea
#define XWimp_PlotIconWithBackgroundSpriteArea  0x600E2
#undef  Wimp_SetMode
...
#undef  WimpTextOp_GetSplitPoint
#define WimpTextOp_GetSplitPoint                0x3
#undef  WimpTextOp_TruncateWithEllipsis
...
#undef  Service_WimpToolSpritesChanged
...
typedef int                                     wimp_t;
#endif
/* On non-RISC OS, pointer-typed fields in binary format structs must be 32-bit */
#ifndef __riscos__
typedef int wimp_ptr32;
#endif
...
#define wimp_VERSION_RO38                       ((wimp_version_no) 0x17Cu)
      /*Introduces nested windows (standard in RISC OS 4+)*/
...
#define wimp_CHAR_XSIZE                         (16)
      /*Width of system font (in OS U)*/
#define wimp_CHAR_YSIZE                         (32)
      /*Height of system font (in OS U)*/
...
#define wimp_WINDOW_USE_TITLE_VALIDATION_COLOURS ((wimp_extra_window_flags) 0x1u)
      /*RISC OS 4+ Prefer Wimp_WindowUseTitleValidationString*/
...
#define wimp_RELEASE_CLIPBOARD                  ((wimp_release_flags) 0x4u)
...
#define wimptextop_SIZE_COVERED                 ((wimptextop_string_width_flags) 0x80000000u)
      /*RISC OS 4.32+*/
#define wimptextop_VJUSTIFY                     ((wimptextop_paint_flags) 0x10000000u)
#define wimptextop_OFFSETS_FOR_REDRAW           ((wimptextop_paint_flags) 0x20000000u)
 ...
 *                window - nested Wimp / RISC OS 3.8+
 *
 * Input:         open - value of R1 on entry
 *                parent - value of R3 on entry
 *                linkage - value of R4 on entry
 ...
 *                additionally its window nesting - nested Wimp / RISC OS
 *                3.8+
 *
 * Input:         state - Partially filled in on entry
 *
 ...
 *                window - reserved; nested Wimp / RISC OS 3.8+
 *
 * Input:         w - value of R0 on entry
 ...
 *
 * Description:   Sets the caret position - RISC OS 4.36+
 *
 ...
 *
 * Description:   Sets the text selection position - RISC OS 4.36+
 *
 ...
 *
 * Description:   Returns details of the caret's state - RISC OS 4.36+
 *
 * Input:         caret - value of R1 on entry
 *
 ...
 *
 * Description:   Returns details of the text selection - RISC OS 4.36+
 *
 * Input:         selection - value of R1 on entry
 *
 ...
 *                outside an update loop - RISC OS 4+ - deprecated, use
 *                Wimp_PlotIconWithWindowSpriteArea
 *
 * Input:         icon - value of R1 on entry
 ...
 *                area - RISC OS 4+ - deprecated, use
 *                Wimp_PlotIconWithBackgroundSpriteArea
 *
 * Input:         icon - value of R1 on entry
 *                x_origin - value of R3 on entry
 *                y_origin - value of R4 on entry
...
/* ------------------------------------------------------------------------
 * Function:      wimp_plot_icon_with_window_sprite_area()
 *
 * Description:   Plots an icon taking the context from a window if used
 *                outside an update loop - RISC OS 4+
 *
 * Input:         icon - value of R1 on entry
 *                flags - value of R3 on entry
 *                x_origin - value of R4 on entry
 *                y_origin - value of R5 on entry
 *                window - value of R6 on entry
 *                sprite_area - value of R7 on entry
 *
 * Other notes:   Calls SWI 0x400E2 with R2 = 0x5441534B, R3 |= 0x0.
 */

extern os_error *xwimp_plot_icon_with_window_sprite_area (wimp_icon const *icon,
      wimp_plot_icon_flags flags,
      int x_origin,
      int y_origin,
      wimp_w window,
      osspriteop_area const *sprite_area);
extern void wimp_plot_icon_with_window_sprite_area (wimp_icon const *icon,
      wimp_plot_icon_flags flags,
      int x_origin,
      int y_origin,
      wimp_w window,
...
/* ------------------------------------------------------------------------
 * Function:      wimp_plot_icon_with_background_sprite_area()
 *
 * Description:   Plots an icon taking the context from a window if used
 *                outside an update loop and using the specified sprite
 *                area - RISC OS 4+
 ...
 *                sprite_area - value of R7 on entry
 *
 * Other notes:   Calls SWI 0x400E2 with R2 = 0x5441534B, R3 |= 0x1.
 */

extern os_error *xwimp_plot_icon_with_background_sprite_area (wimp_icon const *icon,
      wimp_plot_icon_flags flags,
      int x_origin,
      int y_origin,
      wimp_colour background_colour,
      osspriteop_area const *sprite_area);
extern void wimp_plot_icon_with_background_sprite_area (wimp_icon const *icon,
      wimp_plot_icon_flags flags,
      int x_origin,
      int y_origin,
      wimp_colour background_colour,
 ...
 *
 * Description:   Read the command window state - RISC OS Select
 *
...
/* ------------------------------------------------------------------------
 * Function:      wimptextop_get_split_point()
 *
 * Description:   Read a split point by limiting to a given length - RISC
 *                OS 6
 *
 * Input:         s - value of R1 on entry
 *                width - value of R2 on entry
 *                split_char - value of R3 on entry
 *
 * Output:        split_point - value of R0 on exit (X version only)
 *
 * Returns:       R0 (non-X version only)
 *
 * Other notes:   Calls SWI 0x400F9 with R0 = 0x3.
 */

extern os_error *xwimptextop_get_split_point (char const *s,
      int width,
      int split_char,
      char **split_point);
extern char *wimptextop_get_split_point (char const *s,
      int width,
      int split_char);

/* ------------------------------------------------------------------------
 * Function:      wimptextop_truncate_with_ellipsis()
 *
 * Description:   Truncate a string by using an ellipsis character for the
 *                currently selected alphabet, or three full stops - RISC
 *                OS 6
 *
 * Input:         s - value of R1 on entry
 *                out_buf - value of R2 on entry
 *                buf_size - value of R3 on entry
 *                width - value of R4 on entry
 *
 * Output:        buf_size_required - value of R0 on exit (X version only)
 *
 * Returns:       R0 (non-X version only)
 *
 * Other notes:   Calls SWI 0x400F9 with R0 = 0x4.
 */

extern os_error *xwimptextop_truncate_with_ellipsis (char const *s,
      char *out_buf,
      int buf_size,
      int width,
      int *buf_size_required);
extern int wimptextop_truncate_with_ellipsis (char const *s,
      char *out_buf,
      int buf_size,
      int width);
 ...
 *
 * Description:   Configure window auto scrolling - RISC OS 4+
 *
 * Input:         flags - value of R0 on entry
...
/* ------------------------------------------------------------------------
 * Function:      service_wimp_tool_sprites_changed()
 *
 * Description:   Tool sprite area is changed
 *
 * Input:         tool_area - value of R2 on entry
 *
 * Other notes:   Calls SWI 0x30 with R1 = 0x400C3.
 */

extern os_error *xservice_wimp_tool_sprites_changed (osspriteop_area const *tool_area);
extern void service_wimp_tool_sprites_changed (osspriteop_area const *tool_area);
